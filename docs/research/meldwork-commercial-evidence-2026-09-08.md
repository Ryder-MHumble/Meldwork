...
- 2026-09-09 归档复核：来源清单 30 项均有本地正文和元数据文件，但清单的 `sha256` 与所保留 `.txt` 正文字节均不匹配。现有元数据未说明哈希对象，无法判断是否针对未保留的原始 HTTP 响应；保留原值，不将其宣称为正文完整性验证。引用依据仍为正文短摘及原始链接，抓取日期保持 2026-09-08，不伪装为本次重新联网抓取。
