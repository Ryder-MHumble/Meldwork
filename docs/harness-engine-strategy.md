...
当前实现会让所有目标 Agent 按完整轮次顺序发言，默认 6 轮，固定模式最多 10 轮；显式“不限轮数”模式没有整体运行时限，只在达成共识或用户手动停止时结束，同时保留单个 Agent 的独立 watchdog 超时。HTTP 401/403 和兼容性失败会在首次失败后结束该 Agent 当前槽位，保留群聊配置，并要求用户在修复配置后显式重试或替换；网络和限流失败才会使用有界退避重试。它仍是顺序轮询的 MVP，存在四个结构问题：
