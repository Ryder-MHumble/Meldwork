...
当前实现会让所有目标 Agent 按完整轮次顺序发言，默认 6 轮，固定模式最多 10 轮；显式“不限轮数”模式没有整体运行时限，只在达成共识或用户手动停止时结束，同时保留单个 Agent 的独立 watchdog 超时。HTTP 401 会在原调用后自动重试 3 次，仍失败则把脱敏后的基础设施修复任务交给下一个参与 Agent，并定向重置失败 Agent 的原生 Session；修复后再验证 3 次，持续 401 才把该 Agent 从当前群聊移除。用户停止或应用关闭会优先取消整条恢复链，不触发误删。它仍是顺序轮询的 MVP，存在四个结构问题：
