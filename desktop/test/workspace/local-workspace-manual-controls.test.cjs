...
const { executable, within } = require('../support/cli-adapters-test-helpers.cjs')
...
function pendingGate(workspace, timeoutMs = 3000) {
  ...
  return {
    promise: within(seen.promise, timeoutMs),
...
test('LocalWorkspace persists and resumes ACP permission approval and rejection Gates', async (t) => {
  for (const scenario of [
    ...
    await t.test(scenario.status, async (subtest) => {
      ...
      await workspace.refreshAgents()
      const group = workspace.createGroup({
        name: `ACP permission ${scenario.status}`,
        agentKinds: ['kimi'],
        workdir: directory,
        allowWrite: false,
      })
