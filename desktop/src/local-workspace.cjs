...
function isTracedAgentTerminalMessage(message) {
  return message?.role === 'system'
    && Object.hasOwn(AGENT_LABELS, message.agentKind)
    ...
    for (const record of records) {
      ...
      for (const agentRun of Array.isArray(record.agentRuns) ? record.agentRuns : []) {
        ...
        if (!RECOVERABLE_AGENT_STATUSES.has(status)
            || !Object.hasOwn(AGENT_LABELS, agentRun?.kind)) continue
        ...
        if (!trace) continue
        const label = AGENT_LABELS[agentRun.kind] || agentRun.kind
    ...
    const recent = packContextEntries(
      this.recentTranscriptEntries(groupId, afterAgentKind)
        ...
        return {
          ...
          text: traced
            ? evidenceCapsuleText(message, AGENT_LABELS[message.agentKind])
