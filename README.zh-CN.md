...
Meldwork 是一个源码可见的 Electron 桌面应用和 AI Agent 编排工作空间，面向使用多个 AI Agent 的开发者、研究人员和技术团队。它把 Codex、Claude Code、Gemini CLI、OpenCode、Qwen Code 等本地 Agent CLI 放进同一个本地优先 AI Agent 工作空间，支持直接会话、并发回复、有证据可追溯（evidence-aware）运行和人类在环（human-in-the-loop）复核。
...
4. **复核并采用**结果、证据和待处理的人工审批门（Human Gate），再改变工作区。
