...
const historicalGroupRun = computed(() => {
  ...
  const targetKinds = scopedTargets
  const controllerTerminalKeys = new Set([
    'system.autoTimeout', 'system.autoRoundLimit', 'system.autoStopped',
  ])
  if ([...controllerTerminalKeys].some(key => systemKeys.has(key))) return null
  ...
  if (!targetKinds.length || targetKinds.some(kind => !visibleTerminalKinds.has(kind))) return null
