...
describe('RoundRelay workbench', () => {
  ...
  it('uses compact gradient transitions for discovery and first-run onboarding', () => {
    ...
    expect(source).toMatch(/\.agent-discovery-leave-active\s*\{[^}]*transition:\s*opacity 0\.68s cubic-bezier\(0\.4, 0, 0\.2, 1\);/s)
    expect(source).toMatch(/\.agent-discovery-leave-active \.pixel-blast-container\s*\{[^}]*opacity 0\.52s cubic-bezier\(0\.4, 0, 0\.2, 1\),\s*transform 0\.68s cubic-bezier\(0\.4, 0, 0\.2, 1\);/s)
    expect(source).toMatch(/\.agent-discovery-leave-to \.agent-discovery-content\s*\{[^}]*opacity:\s*0;[^}]*transform:\s*translateY\(-10px\) scale\(0\.99\);/s)
    expect(source).toMatch(/@media \(prefers-reduced-motion: reduce\)\s*\{[^}]*\.agent-discovery-leave-active,[^{]+\{[^}]*transition-duration:\s*0\.12s;/s)
